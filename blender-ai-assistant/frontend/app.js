...
const elements = {
    // 状态栏
    blenderDot: document.getElementById('blenderDot'),
    blenderText: document.getElementById('blenderText'),
...
function addCodeMessage(code, step = null, explanation = null) {
    const header = step ? `步骤 ${step} - ${explanation || '生成代码'}` : '生成的 Blender Python 代码：';
...
function addScreenshot(base64Data) {
    const messageDiv = createMessageElement('assistant', 'Blender 截图：');
...
function updateConnectionStatus(connected) {
    if (connected) {
        elements.blenderDot.classList.add('connected');
        elements.blenderText.textContent = 'Blender: 已连接';
        elements.sendBtn.disabled = false;
    } else {
        elements.blenderDot.classList.remove('connected');
        elements.blenderText.textContent = 'Blender: 断开连接';
...
function loadSTLFromBase64(base64Data) {
    ...
    try {
        ...
        // 重要：修复坐标轴对齐问题
        // Blender 使用 Z 轴向上，Three.js 使用 Y 轴向上
        ...
        // 显示预期尺寸（来自用户指令）
        console.log('📋 预期尺寸 (Blender XYZ): x=2, y=4, z=6');
...
function downloadSTL() {
    ...
    try {
        ...
        a.href = url;
        a.download = `blender_model_${Date.now()}.stl`;
...
function requestSTLExport() {
    ...
    exportTimeout = setTimeout(() => {
        if (isExporting) {
            cancelExport();
            showToast('导出超时，请检查 Blender 连接', 'error');
        }
    }, 30000);
...
function downloadModelFile(base64Data, format) {
    try {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `blender_model_${Date.now()}${ext}`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
